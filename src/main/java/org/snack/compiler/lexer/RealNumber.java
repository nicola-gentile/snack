...
public record RealNumber(BigDecimal value) implements Token {
    ...
    public boolean fitFloat32() {
        return value.unscaledValue().bitLength() <= FLOAT32_MANTIX_BIT_LENGTH &&
                value.scale() <= (1 << FLOAT32_EXPONENT_BIT_LENGTH);
    }

    public boolean fitFloat64() {
        return value.unscaledValue().bitLength() <= FLOAT64_MANTIX_BIT_LENGTH &&
                value.scale() <= (1 << FLOAT64_EXPONENT_BIT_LENGTH);
    }
